    ...
    Employee.find({ "employee.managerDetails": emp._id })
        ...
        .then(data => {
            if (!data || data === 0) {
                ...
                res.status(200).json({ message: `employees under the manager ${emp.profile.firstName} : ${data.length} `, arr: data })
            }
        }).catch(error => {
            console.log(error);
            res.status(500).json({ message: "error occurred to display the team to the higher manager" });
        });
}
...
function showTeamToOtherManager(req, res, emp) {
    Employee.find(
        ...
        .then((data) => {
            if (!data || data.length === 0) {
                res.status(404).json({ message: "Don't have any employee under this manager" });
            } else {
                res
                    .status(200)
                    .json({
        ...
        .catch((error) => {
            console.error(error);
            res.status(500).json({ message: "error occured getting details of other manager employees " });
        });
}
...
function showTeamToEmployeeUnderSameManager(req, res, data) {
    Employee.find(
        ...
        .then((emp) => {
            if (!emp || emp.length === 0) {
                res.status(404).json({ message: `Don't have any employee under the manager ${data.profile.firstName}` });
            } else {
                res
                    .status(200)
                    .json({
        ...
        .catch((error) => {
            console.error(error);
            res.status(500).json({ message: "An error occurred in showing the team to an employee under his/her manager." });
        });
}
